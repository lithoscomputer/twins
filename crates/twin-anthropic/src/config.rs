...
pub struct Config {
    ...
    /// Override the generation path for a gateway. Defaults to `/v1/messages`.
...
impl Config {
    ...
    pub fn from_lookup(lookup: &dyn Fn(&str) -> Option<String>) -> Result<Self> {
        ...
        let upstream_api_key = lookup(UPSTREAM_API_KEY_ENV)
            .filter(|value| !value.is_empty())
            .or_else(|| lookup(ANTHROPIC_API_KEY_ENV).filter(|value| !value.is_empty()));
    ...
    pub fn validate(&self) -> Result<()> {
        if self.mode == Mode::ProxyRecord {
            anyhow::ensure!(
                ...
                "proxy-record mode requires {UPSTREAM_API_KEY_ENV} or {ANTHROPIC_API_KEY_ENV}"
            );
            anyhow::ensure!(
