...
impl LiveOptions {
    fn from_env() -> Result<Option<Self>> {
        let Some(api_key) = non_empty_env(OPENAI_API_KEY_ENV) else {
            return Ok(None);
        };
        let base_url =
            non_empty_env(LIVE_BASE_URL_ENV).unwrap_or_else(|| DEFAULT_LIVE_BASE_URL.to_owned());
        let model = non_empty_env(LIVE_MODEL_ENV).unwrap_or_else(|| DEFAULT_LIVE_MODEL.to_owned());
        let organization = non_empty_env(OPENAI_ORGANIZATION_ENV);
        let project = non_empty_env(OPENAI_PROJECT_ENV);
